...
Configure model and runtime cache paths using environment variables.
```csharp
...
```csharp
var preset = AsposeLLMApi.LoadPresetFromJson("{\"presetType\":\"Default\"}");
string compactJson = AsposeLLMApi.SavePresetToJson(preset, includeDefaults: false);
```
