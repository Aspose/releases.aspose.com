...
Identified 7 externally visible Aspose.LLM for .NET release change(s).
...
- Added defensive cleanup of orphaned .download files at start of download to prevent IOException on Windows when previous interrupted attempts left partial files blocki...
- Fixed DefaultFormatter to stop truncating responses at the first blank line by removing the artificial '\n\n' EOS token sentinel and relying on native llama.cpp EOS/EO...
- Added managed fallback for models whose GGUF chat templates are not recognized by llama.cpp's built-in templater (e.g., NVIDIA Nemotron-Mini-4B with <extra_id_0>/<extr...
