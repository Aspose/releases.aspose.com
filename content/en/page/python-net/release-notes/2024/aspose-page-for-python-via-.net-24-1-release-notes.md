---
id: "aspose-page-for-python-via-.net-24-1-release-notes"
slug: "aspose-page-for-python-via-.net-24-1-release-notes"
linktitle: "Aspose.Page for Python via .NET 24.1 Release Notes"
title: "Aspose.Page for Python via .NET 24.1 Release Notes"
description: Python APIs to manipulate and process PS, EPS, and XPS files. This page contains new Aspose.Page for Python via .NET features, enhancement, and bug fixes in 2024, version 24.1.
productName: "Aspose.Page for Python via .NET"
weight: 12
type: "repository"
layout: "release"
hideChildren: false
toc: true
family_listing_page_title: "Aspose.Page for Python via .NET 24.1 Release Notes"
---

{{% alert color="primary" %}}

This page contains Release Notes for [Aspose.Page for Python via .NET 24.1](https://pypi.org/project/aspose-page/)

{{% /alert %}}

We are pleased to announce the release of the first version of Aspose.Page for Python via .NET 24.1. Aspose.Page is an API for manipulating [PDL](https://en.wikipedia.org/wiki/Page_description_language) formats. The first version of Aspose.Page can:

- Create, edit and save PS/EPS and XPS documents
- Convert XPS document to PDF and image
- Convert PostScript (PS) and Encapsulated PostScript (EPS) files to PDF and image
### **XPS Features**
- Create and edit XPS document
- Add or remove pages of XPS document
- Work with canvases, paths and glyphs elements
- Create vector graphics shapes (Path element) using a set of primitives (elliptical arcs, Bezier curve segments and straight line segments)
- Create text strings (Glyphs element)
- Group elements (Canvas element) to manipulate a group as a whole
- Manipulate the appearance of graphics and text strings
- Use brushes of different types including solid color brush, image brush, visual brush etc
- Specify colors in different color spaces including sRGB, scRGB and any space based on ICC profile
- Work with multiple documents inside an XPS document
- Preserve print tickets in opened XPS document if any
- Add default print tickets into new XPS document or an opened one if there are no tickets in it
- Perform cross-package operations, e.g. add/insert an element from another document (XPS package), assign a property value created in another document's context, create elements/property values based on file resources created in another document's context, add/insert a page from another document
- Get a job/document/page print ticket object and link it to another job/document/page
- Manipulate print tickets using the Print Schema model classes
- Save XPS documents as raster images
- Save XPS documents to PDF format
- Save XPS documents to PostScript
- Manipulate hyperlinks associated with XPS elements
- Save PDF to stream
- Manipulate, save and convert XPS outline items.
### **EPS Features**
- Supports PostScript language levels 1-3 with an exception of font types: Type2 (CFF), Type14 (Chameleon), Types 9, 10, 11
- Create PS document
- Add page to PS document
- Work with graphics states (transformations and clips)
- Create vector graphics shapes using a set of primitives (elliptical arcs, Bezier curve segments and straight line segments)
- Create text strings
- Add images including transparent ones
- Save as PDF
- Save as raster image
- Resize and crop EPS images
- Convert raster images to EPS
- Add, edit and save XMP metadata in EPS files

<!--## **Improvements and Changes**-->
