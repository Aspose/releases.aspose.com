---
id: "aspose-tasks-for-python-net-26-9-release-notes"
slug: "aspose-tasks-for-python-net-26-9-release-notes"
linktitle: "Aspose.Tasks for Python via .NET 26.9 Release Notes"
title: "Aspose.Tasks for Python via .NET 26.9 Release Notes"
weight: 92
description: "The page contains the release notes for Aspose.Tasks for Python via .NET 26.9."
type: "repository"
layout: "release"
hideChildren: false
toc: false
family_listing_page_title: "Aspose.Tasks for Python via .NET 26.9 Release Notes"
menuItemWithNoContent: false
---

{{% alert color="primary" %}}

This page contains release notes information for [Aspose.Tasks for Python via .Net 26.9](https://pypi.org/project/aspose-tasks/26.9.0/).
...
| :- | :- |
| aspose.tasks.Baseline.timephased_data | Gets or sets a TimephasedDataCollection instance for this object. |
...
baseline = next(
    b for b in task.baselines
    if b.baseline_number == tasks.BaselineType.BASELINE
)
baseline_td = baseline.timephased_data

# Or use get_timephased_data method
baseline_td = task.get_timephased_data(start, finish, tasks.TimephasedDataType.TASK_BASELINE_WORK)
```
...
# Get baseline timephased data via Baseline object
baseline = next(
    b for b in resource.baselines
    if b.baseline_number == tasks.BaselineType.BASELINE
)
baseline_td = baseline.timephased_data

# Or use get_timephased_data method
baseline_td = resource.get_timephased_data(start, finish, tasks.TimephasedDataType.RESOURCE_BASELINE_WORK)
```
...
# Get baseline timephased data via Baseline object
baseline = next(
    b for b in assignment.baselines
    if b.baseline_number == tasks.BaselineType.BASELINE
)
baseline_td = baseline.timephased_data

# Or use get_timephased_data method
baseline_td = assignment.get_timephased_data(start, finish, tasks.TimephasedDataType.ASSIGNMENT_BASELINE_WORK)
```
...
all_td = task.timephased_data

# Filter baseline data
baseline_td = [
    td for td in all_td
    if td.timephased_data_type == tasks.TimephasedDataType.TASK_BASELINE_WORK
]
```
...
# Or use get_timephased_data for filtered results
baseline_td = task.get_timephased_data(
    task.start,
    task.finish,
    tasks.TimephasedDataType.TASK_BASELINE_WORK,
)
```
