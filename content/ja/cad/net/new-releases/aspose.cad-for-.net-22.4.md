---
title: "C#、ASP.NET アプリ用の DXF を TIFF に、DWG を WMF API に変換"
description: "AutoCAD® DXF から TIFF への変換、DWG から WMF へのエクスポート、埋め込まれた画像データからの画像のエクスポート、改善された DWG レンダリングでのメモリ管理が改善された C# .NET API。"
keywords: ""
...
<h4>リリースノート</h4><div><a href="https://docs.aspose.com/cad/net/aspose-cad-for-net-22-4-release-notes/">https://docs.aspose.com/cad/net/aspose-cad-for-net-22-4-release-notes/</a></div>
{{< /Releases/ReleasesFileFeatures >}}
...
AutoCAD® DXF を TIFF (ラスター グラフィックス) 形式に変換しているときに、メモリ リークが発生していました。そのため、API のこのリリースでは、DXF から TIFF への変換のメモリ管理が改善されています。

# DWG から WMF へのエクスポートの改善

AutoCAD® `DWG` の `WMF` (Windows メタファイル) へのエクスポートが改善されました。

# 埋め込み画像のエクスポート

埋め込まれた画像データから画像をエクスポートする機能。

# `DWG` レンダリングの改善

「DWG」が色なしでレンダリングされていることが確認されました。この問題は、API のこのリリースで解決されました。

> このリリースの機能、拡張機能、バグ修正の完全なリストについては、[Aspose.CAD for .NET 22.4 - Release Notes](https://docs.aspose.com/cad/net/aspose-cad-for-net-22-4-release-notes/) にアクセスしてください。
