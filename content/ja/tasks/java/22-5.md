---
title: "Aspose.tasks 22.5 | Microsoftプロジェクト処理Java API" 
...
[Aspose.Tasks](https://products.aspose.com/tasks/java) Microsoftプロジェクトファイルを操作するJavaプロジェクト管理API。開発者は、カレンダー情報だけでなく、プロジェクトのメインプロパティとデフォルトプロパティを定義できます。また、APIは、ユーザーの介入なしでタスク、リソース、その他のデータを簡単に追加および再計算できるようにしながら、平日とカレンダーの例外を指定する機能を提供します。

## Microsoftプロジェクトファイル処理
 - マイクロソフトプロジェクトを読み、変更し、書きますか？ドキュメント。
