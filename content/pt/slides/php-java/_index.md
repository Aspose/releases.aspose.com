---
title: "Aspose - Downloads"
description: "Contém arquivos para slides"
keywords: "slides"
family_listing_page_title: "Aspose.Slides para PHP via Java"
family_listing_page_description: "Uma API de processamento de apresentação que permite que os aplicativos PHP manipulem, renderizem e convertam apresentações do PowerPoint sem exigir o aplicativo Microsoft PowerPoint."
family_listing_page_iconurl: "https://www.aspose.cloud/templates/aspose/img/products/slides/aspose_slides-for-php.svg"
homepage_package_type: "Composer"
homepage_package_link: "https://packagist.org/packages/aspose/slides"
weight:  99
---

{{< dbToolbar link="https://forum.aspose.com/c/slides" linktext=" Support Forum " >}}
{{< ProductPageWrapper >}}

<!-- ProductPageContent-->
{{< Common/wrapper class="col-md-12" >}}
{{< Common/wrapper class="panel-body downloadfilebody" >}}
{{< Common/h1 text="Aspose.Slides para PHP via Java" >}}
{{< Common/paragraph>}}
UMA
{{< Common/link href="https://products.aspose.com/slides/php-java/" text="API de processamento de apresentação"  >}}que permite que os aplicativos PHP manipulem, renderizem e convertam apresentações do PowerPoint sem exigir o aplicativo Microsoft PowerPoint.
{{< Common/h4 text="API PHP para apresentações"  >}} 
{{< Common/ul>}}
   {{< Common/li >}} Complete control over presentation elements via comprehensive document object model. {{< /Common/li >}}
   {{< Common/li >}} Conversion of PowerPoint presentation, template & slideshow file formats to 25+ supported formats. {{< /Common/li >}}
   {{< Common/li >}} Add, manipulate & format charts, shapes, slides, SmartArt, tables & text. {{< /Common/li >}}
{{< /Common/ul>}}

{{< /Common/paragraph>}}
{{< Common/hr >}}
{{< Common/h4 text="Baixar aspose/slides do Composer"  >}}
{{< Common/paragraph class="package-instructions">}}
aspose/slides está disponível no Packagist como o
{{< Common/link href="https://packagist.org/packages/aspose/slides" text="aspose/slides"  >}}pacote. Execute o seguinte comando:
{{< /Common/paragraph>}}
{{< consolebox/consoleboxwrapper id="composer" >}}
       {{< consolebox/spantext text=" " >}}
       {{< consolebox/textarea id="composer" >}}composer require aspose/slides {{< /consolebox/textarea >}}
{{< /consolebox/consoleboxwrapper >}}
{{< Common/h4 text="Notas de lançamento"  >}}
{{< Common/release-notes-link family="slides" product="php-java" href="https://docs.aspose.com/slides/php-java/release-notes/" text="https://docs.aspose.com/slides/php-java/release-notes/"  >}}
{{< /Common/wrapper >}}
{{< /Common/wrapper >}}

<!-- /ProductPageContent-->

<!-- ReleasesListProductPage-->
   {{< Releases/ReleasesListProductPage shownested="false"  directdownload="true" family="slides" product="php-java" >}}
<!-- /ReleasesListProductPage-->

{{< /ProductPageWrapper >}}
