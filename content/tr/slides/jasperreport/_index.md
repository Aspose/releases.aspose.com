---
title: "PowerPoint için İhracatçı | Aspose.Slides for JasperReports"
description: "Microsoft PowerPoint sunumları için JasperReports ve JasperServer Exporter'ı indirin. Raporlarınızı Microsoft PowerPoint sunumlarına (PPT, PPTX) ve Microsoft PowerPoint slayt gösterilerine (PPS, PPSX) aktarın."
keywords: "JasperReports Exporter for PowerPoint "
family_listing_page_title: "Aspose.Slides for JasperReports"
family_listing_page_description: "Aspose.Slides for JasperReports, raporları Microsoft PowerPoint'e aktarmanıza izin veren esnek bir JasperReports ve JasperServer uzantısıdır. JasperServer, raporları PowerPoint sunumları olarak dışa aktarmak için yerleşik bir yeteneğe sahip değildir, ancak bu uzantı ile JasperServer raporları, Microsoft PowerPoint kullanılmadan PPT, PPS, PPTX ve PPSX sunum biçimlerine aktarılabilir."
family_listing_page_iconurl: "https://www.aspose.cloud/templates/aspose/App_Themes/V3/images/slides/272x272/aspose_slides-for-jasperreports-min.png"
family_listing_page_selfHosted: "1"
family_listing_page_type: "2"
family_listing_page_venture: "4"
family_listing_page_package: ""
tags: ['Aspose', 'Aspose.Total', 'Aspose.Slides', 'Slides', 'Assembly', 'component', 'Conholdate', 'Conholdate.Total', 'convert', 'converter', 'load', 'save', 'edit', 'update', 'core', 'cross-platform', 'Fast', 'Framework', 'Library', 'macOS', 'Microsoft', 'mono', 'mvc', 'Office', 'opendocument', 'openoffice', 'print', 'render', 'Standard', 'Visual-Studio', 'VisualStudio', 'Animation', 'jasperreports', 'PPT', 'PPS', 'PPTX', 'PPSX', 'PDF', 'HTML']
weight:  7
---

{{< dbToolbar link="https://forum.aspose.com/c/slides" linktext=" Support Forum " >}}

{{< Common/h3 text="JasperReports®'u Sunum Formatlarına Aktarmak için Aspose.Slides'i İndirin"  >}}
{{< Common/paragraph class="package-instructions">}}

{{< Common/link href="https://products.aspose.com/slides/jasperreports/" text="Aspose.Slides for JasperReports"  >}}raporları JasperReports® ve JasperServer®'dan Microsoft PowerPoint® ve diğer desteklenen dosya biçimlerine aktarmanıza yardımcı olan hafif bir uzantıdır.
{{< Common/release-notes-link family="slides" product="jasperreports" href="https://docs.aspose.com/slides/jasperreports/release-notes/" text="https://docs.aspose.com/slides/jasperreports/release-notes/"  >}}
{{< /Common/paragraph>}}
{{< Common/hr >}}

{{KÖPRÜ1}} | {{KÖPRÜ2}} | {{KÖPRÜ3}} | {{KÖPRÜ4}} | {{KÖPRÜ5}} | {{KÖPRÜ6}} | {{KÖPRÜ7}} | {{KÖPRÜ8}}

### Sunum JasperReports® Entegrasyon Özellikleri

- Raporları PPT, PPTX, HTML ve PDF formatlarına aktarın.
- Raporları doğru bir düzen ile PPT ve PPS formatlarında dışa aktarın.
- JasperReport ve JasperServer ile entegrasyon.

#### Desteklenen Okuma ve Yazma Formatları

**Microsoft PowerPoint®:** PPT, PPS, PPTX, PPSX

#### Desteklenen Yalnızca Çıktı Biçimleri

**Sabit Düzen:** PDF\
**Web:** HTML

### Sistem gereksinimleri

#### Desteklenen Microsoft PowerPoint® Sürümleri
- Microsoft PowerPoint® 97
- Microsoft PowerPoint® 2000
- Microsoft PowerPoint® XP
- Microsoft PowerPoint® 2003
- Microsoft PowerPoint® 2007
- Microsoft PowerPoint® 2010
- Microsoft PowerPoint® 2013
- Microsoft PowerPoint® 2016
- Microsoft PowerPoint® 2019
- MAC için Microsoft PowerPoint®
- Office® 365

#### JasperReports 17.5 ve önceki sürümleri için Aspose.Slides
- Java 2 Platform Standard Edition 1.4 veya üstü.
- JasperReports 2.0, 3.0 veya üstü

{{KÖPRÜ1}} | {{KÖPRÜ2}} | {{KÖPRÜ3}} | {{KÖPRÜ4}} | {{KÖPRÜ5}} | {{KÖPRÜ6}} | {{KÖPRÜ7}} | {{KÖPRÜ8}}

{{< Common/hr >}}

{{< Tags/TagsWrapper >}}
 {{< Tags/TagsList >}}
{{< /Tags/TagsWrapper >}}

{{< Common/hr >}}

{{< ProductPageWrapper >}}
<!-- ReleasesListProductPage-->
   {{< Releases/ReleasesListProductPage shownested="false"  directdownload="true" family="slides" product="jasperreports" >}}
<!-- /ReleasesListProductPage-->
{{< Common/hr >}}
{{< /ProductPageWrapper >}}

